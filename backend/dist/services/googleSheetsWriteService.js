...
class GoogleSheetsWriteService {
    ...
    initSync() {
        try {
            ...
            if (!this.credentials) {
                logger_1.logger.warn('[SheetsWrite] No service account credentials found. Write features disabled.');
                return;
            }
            ...
            logger_1.logger.info('[SheetsWrite] ✅ Service initialized with service account:', this.credentials.client_email);
        }
        catch (error) {
     ...
     */
    async updateCell(spreadsheetId, sheetName, row, column, value) {
        if (!this.isReady()) {
     ...
     */
    async validateWriteAccess(spreadsheetId) {
        if (!this.isReady()) {
            return {
                valid: false,

...
export class WeatherTool {
    ...
    async getWeather(location: string = 'Tangerang Selatan'): Promise<WeatherData> {
     ...
     */
    async getWeatherForecast(location: string, days: number = 0): Promise<WeatherData> {
        try {
            ...
            if (!geoResponse.ok) {
                logger.warn('Geocoding API error, returning mock data', { status: geoResponse.status });
            ...
            if (!geoData.results || geoData.results.length === 0) {
                logger.warn('Location not found, returning mock data', { location });
                return this.getMockWeather(location);
            }
            ...
            if (!weatherResponse.ok) {
                logger.warn('Weather API error, returning mock data', { status: weatherResponse.status });
                return this.getMockWeather(location);
            }
            ...
            logger.error('Weather tool error', { error: error.message });
            return this.getMockWeather(location);
        }
    }

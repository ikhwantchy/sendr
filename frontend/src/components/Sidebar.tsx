...
export default function Sidebar() {
        ...
        <>
            <div className="flex items-center justify-between px-4 py-6 border-b border-zinc-800/50">
                ...
                <button
                    ...
                    className="hidden md:flex p-1.5 hover:bg-zinc-800/50 rounded-md text-zinc-500 hover:text-zinc-100 transition-colors"
                >
                    ...
                    return (
                        <Link
                            ...
                            className={`group flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 ${isActive
                                ? 'bg-zinc-900 text-zinc-100 shadow-[0_1px_0_0_rgba(255,255,255,0.05)]'
                ...
                <div className="p-4 border-t border-zinc-900 bg-zinc-950 relative">
                    ...
                    >
                        <div className="w-8 h-8 rounded-full overflow-hidden flex-shrink-0 border border-zinc-800 group-hover:border-zinc-700 transition-colors bg-zinc-800">
                            <div className="w-full h-full flex items-center justify-center bg-zinc-900 text-zinc-500 uppercase font-bold text-xs">
                                {user.name[0]}
                            </div>
                        </div>
                        {isExpanded && (
                    ...
                    {showUserMenu && (
                        <div className="absolute bottom-full left-4 right-4 mb-2 p-1 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl z-[60] overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-200">
                            <button
    ...
    return (
        <>
            <div
                className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-50 md:hidden transition-opacity duration-300 ${isMobileOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
                    }`}
                onClick={() => setIsMobileOpen(false)}
            />
            ...
            {!isMobileOpen && (
                <button
                    ...
                    className="md:hidden fixed top-4 right-4 z-[45] p-2 bg-zinc-950 border border-zinc-900 rounded-lg text-zinc-400"
                >
            ...
            <aside
                className={`fixed left-0 top-0 h-full bg-zinc-950 border-r border-zinc-900 z-[55] transition-all duration-300 ease-in-out flex flex-col ${isExpanded ? 'w-64' : 'w-20'
                    } hidden md:flex`}
            >
                <SidebarContent />
            </aside>
            ...
            <aside
                className={`fixed left-0 top-0 h-full bg-zinc-950 z-[60] transition-transform duration-300 ease-in-out flex flex-col w-64 md:hidden ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                <SidebarContent />
            </aside>

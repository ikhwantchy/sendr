...
exports.aiSheetUpdaterService = void 0;
const uuid_1 = require("uuid");
const connection_1 = require("../database/connection");
const logger_1 = require("../utils/logger");
const googleSheetsWriteService_1 = require("./googleSheetsWriteService");
class AISheetUpdaterService {
    /**
     * Default value mappings for common use cases
     */
    static DEFAULT_EVENT_MAPPINGS = [
        {
    ...
    async createConfig(config) {
        try {
            ...
            const spreadsheetId = googleSheetsWriteService_1.googleSheetsWriteService.extractSpreadsheetId(config.spreadsheet_url);
            if (!spreadsheetId) {
            ...
            logger_1.logger.info('[AISheetUpdater] Created config:', { id, name: config.name, mode });
            return { success: true, id };
        }
        catch (error) {
            logger_1.logger.error('[AISheetUpdater] Error creating config:', error.message);
    ...
    async getConfigsByBot(botId) {
        try {
            const result = await (0, connection_1.query)('SELECT * FROM ai_sheet_updaters WHERE bot_id = ? ORDER BY created_at DESC', [botId]);
            ...
            logger_1.logger.error('[AISheetUpdater] Error getting configs:', error.message);
            return [];
        }
    }
    /**
    ...
    async getConfig(configId) {
        try {
            const result = await (0, connection_1.query)('SELECT * FROM ai_sheet_updaters WHERE id = ?', [configId]);
            ...
            logger_1.logger.error('[AISheetUpdater] Error getting config:', error.message);
            return null;
        }
    }
    /**
    ...
    async updateConfig(configId, updates) {
        try {
            ...
            const spreadsheetId = updates.spreadsheet_url
                ? googleSheetsWriteService_1.googleSheetsWriteService.extractSpreadsheetId(updates.spreadsheet_url)
            ...
            logger_1.logger.error('[AISheetUpdater] Error updating config:', error.message);
            return { success: false, error: error.message };
        }
    }
    /**
    ...
    async deleteConfig(configId) {
        try {
            await (0, connection_1.query)('DELETE FROM ai_sheet_updaters WHERE id = ?', [configId]);
            return { success: true };
        }
        catch (error) {
            return { success: false, error: error.message };
     ...
     */
    async getConfigsByTargetJid(botId, targetJid) {
        try {
            ...
            return configs.filter((config) => config.target_jids && config.target_jids.includes(targetJid));
        }
        catch (error) {
            logger_1.logger.error('[AISheetUpdater] Error getting configs by target JID:', error.message);
            return [];
        }
    }
    /**
     ...
     */
    async deleteConfigsByTargetJid(botId, targetJid) {
        try {
            ...
            for (const config of configs) {
                ...
                if (config.target_jids && config.target_jids.length === 1) {
                    await (0, connection_1.query)('DELETE FROM ai_sheet_updaters WHERE id = ?', [config.id]);
                    deleted++;
                }
                    ...
                    await (0, connection_1.query)('UPDATE ai_sheet_updaters SET target_jids = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [JSON.stringify(newTargetJids), config.id]);
                }
            }
            logger_1.logger.info(`[AISheetUpdater] Deleted/updated ${deleted} configs for target JID: ${targetJid}`);
            return { success: true, deleted };
        }
        catch (error) {
            logger_1.logger.error('[AISheetUpdater] Error deleting configs by target JID:', error.message);
    ...
    async toggleConfig(configId) {
        try {
            ...
            await (0, connection_1.query)('UPDATE ai_sheet_updaters SET is_enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [newState ? 1 : 0, configId]);
     ...
     */
    async classifyMessage(message, valueMappings, aiInstructions) {
...
Respond with ONLY the classification word (e.g., CONFIRMED, DECLINED, MAYBE, UNKNOWN). No explanation.`;
            ...
            const matchedMapping = valueMappings.find(m => m.value.toUpperCase() === classification);
            if (matchedMapping) {
                return {
            ...
            logger_1.logger.error('[AISheetUpdater] AI classification error:', error.message);
            return {
     ...
     */
    async extractDataFromMessage(message, columnSchema, senderPhone, senderName, aiInstructions) {
        try {
            const data = {};
            const aiExtractionFields = [];
...
- For DATE fields: Convert to YYYY-MM-DD format.
  ...
  "15 februari" = 2026-02-15 (year 2026 if not specified).`;
                try {
                    ...
                    logger_1.logger.error('[AISheetUpdater] Failed to parse AI extraction response:', parseError.message);
            ...
            logger_1.logger.error('[AISheetUpdater] Error extracting data:', error.message);
            return {
                success: false,
                data: {},
                confidence: 0,
     ...
     */
    async determineSmartAction(message, config, phoneExistsInSheet, aiInstructions) {
        try {
...
{"action": "update|create|skip", "reason": "brief explanation"}`;
            ...
            logger_1.logger.error('[AISheetUpdater] Error determining smart action:', error.message);
     ...
     */
    checkTriggerKeywords(message, keywords) {
     ...
     */
    async processMessage(botId, senderPhone, message, senderJid, senderName) {
        try {
            ...
            for (const config of enabledConfigs) {
                ...
                if (!this.checkTriggerKeywords(message, config.trigger_keywords)) {
                    logger_1.logger.debug(`[AISheetUpdater] Message doesn't match trigger keywords for ${config.name}`);
                    continue;
                }
                ...
                const mode = config.mode || 'update';
                logger_1.logger.info(`[AISheetUpdater] Processing with mode: ${mode} for config: ${config.name}`);
                ...
                let findResult = await googleSheetsWriteService_1.googleSheetsWriteService.findRowByValue(config.spreadsheet_id, config.sheet_name, config.match_column, normalizedPhone);
                ...
                if (!findResult.found && isLid) {
                    logger_1.logger.info(`[AISheetUpdater] 🔍 Phone not found (LID detected), trying campaign recipient match...`);
                    // Try to find phone from recent campaign recipients
                    const campaignMatch = await this.findPhoneFromRecentCampaign(botId, config.spreadsheet_id, config.sheet_name, config.match_column);
                    if (campaignMatch.phone && campaignMatch.confidence !== 'none') {
                        logger_1.logger.info(`[AISheetUpdater] ✅ Found from campaign! Phone: ${campaignMatch.phone} (confidence: ${campaignMatch.confidence})`);
                        matchedPhone = campaignMatch.phone;
                        // Re-search with the matched phone
                        findResult = await googleSheetsWriteService_1.googleSheetsWriteService.findRowByValue(config.spreadsheet_id, config.sheet_name, config.match_column, matchedPhone);
                        // Auto-create LID mapping for future use
                        if (findResult.found && senderJid) {
                            try {
                                const { lidPhoneMappingService } = await Promise.resolve().then(() => __importStar(require('./lidPhoneMappingService')));
                                ...
                                logger_1.logger.info(`[AISheetUpdater] 🔗 Auto-created LID mapping: ${lid} → ${matchedPhone}`);
                            }
                            catch (mapErr) {
                                logger_1.logger.warn('[AISheetUpdater] Failed to auto-create LID mapping', { error: mapErr });
                            }
                        }
                    }
                }
                ...
                if (mode === 'smart') {
                    const decision = await this.determineSmartAction(message, config, findResult.found, config.ai_instructions);
                    logger_1.logger.info(`[AISheetUpdater] 🧠 Smart mode decision: ${decision.action} (${decision.reason})`);
                    if (decision.action === 'skip') {
                        logger_1.logger.debug(`[AISheetUpdater] Smart mode decided to skip this message`);
                        continue;
                    }
                ...
                // UPDATE mode logic
                if (!findResult.found) {
                    logger_1.logger.debug(`[AISheetUpdater] Phone ${normalizedPhone} not found in ${config.name}`);
                    continue;
                }
            ...
            logger_1.logger.error('[AISheetUpdater] Error processing message:', error.message);
     ...
     */
    async handleUpdateMode(config, message, matchedPhone) {
        // Classify the message
        const classification = await this.classifyMessage(message, config.value_mappings, config.ai_instructions);
        if (classification.classification === 'UNKNOWN' || classification.classification === 'ERROR') {
            logger_1.logger.debug(`[AISheetUpdater] Could not classify message: "${message}"`);
            return { success: false, message: 'Could not classify message', rowFound: true, mode: 'update' };
        }
        // Update the spreadsheet
        const updateResult = await googleSheetsWriteService_1.googleSheetsWriteService.updateRowByMatch(config.spreadsheet_id, config.sheet_name, config.match_column, matchedPhone, config.update_column, classification.mappedValue);
        if (updateResult.success) {
            // Log the update
            await this.logUpdate(config.id, matchedPhone, message, classification);
            logger_1.logger.info(`[AISheetUpdater] ✅ Updated ${config.sheet_name}: ${matchedPhone} → ${classification.mappedValue}`);
            return {
                success: true,
     ...
     */
    async handleCreateMode(config, message, senderPhone, senderName) {
        if (!config.column_schema || config.column_schema.length === 0) {
            logger_1.logger.warn(`[AISheetUpdater] CREATE mode requires column_schema but none defined for ${config.name}`);
            return {
                success: false,
        ...
        const extraction = await this.extractDataFromMessage(message, config.column_schema, senderPhone, senderName, config.ai_instructions);
        if (!extraction.success) {
            logger_1.logger.warn(`[AISheetUpdater] Failed to extract data: ${extraction.error}`);
            return {
                success: false,
        ...
        try {
            const appendResult = await googleSheetsWriteService_1.googleSheetsWriteService.appendRowAsObject(config.spreadsheet_id, config.sheet_name, extraction.data);
            if (appendResult.success) {
                logger_1.logger.info(`[AISheetUpdater] ✅ Created new row in ${config.sheet_name}:`, extraction.data);
            ...
            logger_1.logger.error('[AISheetUpdater] Error appending row:', error.message);
            return {
                success: false,
     ...
     */
    async findPhoneFromRecentCampaign(botId, spreadsheetId, sheetName, phoneColumn) {
        try {
            // Get recent campaign recipients (last 24 hours) that were successfully sent
            const recentRecipients = await (0, connection_1.query)(`
            ...
            if (!recentRecipients.rows || recentRecipients.rows.length === 0) {
                logger_1.logger.debug('[AISheetUpdater] No recent campaign recipients found');
                return { phone: null, confidence: 'none' };
            }
            logger_1.logger.info('[AISheetUpdater] 📋 Found recent campaign recipients', {
                count: recentRecipients.rows.length,
                phones: recentRecipients.rows.map((r) => r.phone?.slice(-4))
            });
            // Get phones that already have LID mappings
            const existingMappings = await (0, connection_1.query)(`
                SELECT phone FROM lid_phone_mappings WHERE bot_id = ?
            `, [botId]);
            const mappedPhones = new Set(existingMappings.rows.map((r) => this.normalizePhone(r.phone)));
            // Get phones from spreadsheet
            const { headers, rows } = await googleSheetsWriteService_1.googleSheetsWriteService.readSheet(spreadsheetId, sheetName);
            const phoneColumnIndex = headers.findIndex(h => h.toLowerCase().trim() === phoneColumn.toLowerCase().trim());
            ...
            logger_1.logger.info('[AISheetUpdater] 📊 Unmapped candidates from recent campaigns', {
            ...
            logger_1.logger.warn('[AISheetUpdater] ⚠️ Multiple unmapped candidates - using first one', {
            ...
            logger_1.logger.error('[AISheetUpdater] Error finding phone from campaign:', error.message);
            return { phone: null, confidence: 'none' };
        }
    }
    /**
     ...
     */
    async logUpdate(configId, phone, message, classification) {
        try {
            await (0, connection_1.query)(`
            ...
            logger_1.logger.error('[AISheetUpdater] Error logging update:', error.message);
        }
    }
    /**
     * Get update logs for a config
     */
    async getUpdateLogs(configId, limit = 50) {
        try {
...
exports.default = exports.aiSheetUpdaterService;
//# sourceMappingURL=aiSheetUpdaterService.js.map
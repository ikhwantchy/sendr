    ...
    async chat(messages: LLMMessage[], config: LLMConfig): Promise<LLMResponse> {
        try {
            ...
            if (functionCall && enableTools) {
                logger.info('LLM requested function call', { function: functionCall.name });
    ...
    async extractData(
        ...
        try {
            ...
            const schemaDescription = Object.entries(schema.fields)
                .map(([key, field]) =>
                    `- ${key} (${field.type}${field.required ? ', required' : ''}): ${field.description || ''}`
                )
                .join('\n');
...
If a field is not found, use null. Do not include fields that are not in the schema.`;
            ...
            try {
                ...
                logger.error('Failed to parse extracted data', { response: response.content });
                extracted = {};
            }
            ...
            logger.error('Gemini extract error', { error: error.message });
            throw error;
        }
    }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 429) {
                    logger.info('Gemini connection test: Key is valid but quota exceeded (429)');
                    return true;
                }
                logger.warn('Gemini connection test failed', { status: response.status, error });
                return false;
            }

            return true;
        } catch (error: any) {
            logger.error('Gemini connection test exception', {
                error: error.message,
                cause: error.cause,
                stack: error.stack
            });

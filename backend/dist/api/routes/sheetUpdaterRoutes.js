...
router.post('/validate-sheet', async (req, res) => {
    try {
        ...
        const spreadsheetId = googleSheetsWriteService_1.googleSheetsWriteService.extractSpreadsheetId(spreadsheetUrl);
        if (!spreadsheetId) {
 ...
 */
router.get('/configs/:botId', async (req, res) => {
    try {
        const { botId } = req.params;
        const configs = await aiSheetUpdaterService_1.aiSheetUpdaterService.getConfigsByBot(botId);
        res.json({
            success: true,
            data: configs
        });
 ...
 */
router.get('/configs/:botId/by-target/:targetJid', async (req, res) => {
    try {
        const { botId, targetJid } = req.params;
        const configs = await aiSheetUpdaterService_1.aiSheetUpdaterService.getConfigsByTargetJid(botId, decodeURIComponent(targetJid));
        res.json({
            success: true,
            data: configs
        });
 ...
 */
router.delete('/configs/:botId/by-target/:targetJid', async (req, res) => {
    try {
        const { botId, targetJid } = req.params;
        const result = await aiSheetUpdaterService_1.aiSheetUpdaterService.deleteConfigsByTargetJid(botId, decodeURIComponent(targetJid));
        if (!result.success) {
...
router.get('/config/:configId', async (req, res) => {
    try {
        ...
        const config = await aiSheetUpdaterService_1.aiSheetUpdaterService.getConfig(configId);
        if (!config) {
...
router.post('/configs', async (req, res) => {
    try {
        ...
        if (!config.value_mappings || config.value_mappings.length === 0) {
            config.value_mappings = aiSheetUpdaterService_1.aiSheetUpdaterService.constructor.prototype.constructor['DEFAULT_EVENT_MAPPINGS']
                || [
        ...
        const result = await aiSheetUpdaterService_1.aiSheetUpdaterService.createConfig(config);
        if (!result.success) {
            return res.status(400).json({ success: false, error: result.error });
        }
        res.json({
        ...
        logger_1.logger.error('[SheetUpdaterRoutes] Error creating config:', error.message);
        res.status(500).json({ success: false, error: error.message });
    }
});
/**
...
router.put('/config/:configId', async (req, res) => {
    try {
        ...
        const result = await aiSheetUpdaterService_1.aiSheetUpdaterService.updateConfig(configId, updates);
        if (!result.success) {
            return res.status(400).json({ success: false, error: result.error });
        }
...
router.delete('/config/:configId', async (req, res) => {
    try {
        const { configId } = req.params;
        const result = await aiSheetUpdaterService_1.aiSheetUpdaterService.deleteConfig(configId);
        if (!result.success) {
            return res.status(400).json({ success: false, error: result.error });
        }
        res.json({ success: true });
 ...
 */
router.patch('/config/:configId/toggle', async (req, res) => {
    try {
        const { configId } = req.params;
        const result = await aiSheetUpdaterService_1.aiSheetUpdaterService.toggleConfig(configId);
        if (!result.success) {
...
router.post('/test-classify', async (req, res) => {
    try {
        ...
        const result = await aiSheetUpdaterService_1.aiSheetUpdaterService.classifyMessage(message, mappings, aiInstructions);
        res.json({
            success: true,
            data: result
        });
...
router.post('/test-update', async (req, res) => {
    try {
        ...
        const result = await aiSheetUpdaterService_1.aiSheetUpdaterService.processMessage(botId, phone, message);
        res.json({
...
router.get('/logs/:configId', async (req, res) => {
    try {
        ...
        const logs = await aiSheetUpdaterService_1.aiSheetUpdaterService.getUpdateLogs(configId, limit);
        res.json({
            success: true,
            data: logs
        });
 ...
 */
router.get('/default-mappings', (req, res) => {
    res.json({
        success: true,
        data: {
            event: [
...
//# sourceMappingURL=sheetUpdaterRoutes.js.map
    ...
    return (
        <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-xl p-4 sm:p-6 relative overflow-hidden group hover:border-zinc-300 dark:hover:border-zinc-800 transition-colors shadow-sm dark:shadow-none">
            <div className="flex justify-between items-start mb-3 sm:mb-4">
                ...
                {status && (
                    <span className={`text-[9px] sm:text-[10px] font-bold uppercase tracking-wider px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full border ${getStatusBadge()}`}>
                        {status}
                    </span>
                )}
            </div>
            ...
            <div className="space-y-1">
                {loading ? (
                    <div className="h-6 sm:h-8 w-20 sm:w-24 bg-zinc-200 dark:bg-zinc-900 rounded animate-pulse" />
                ) : (
                    <h3 className={`text-xl sm:text-2xl font-bold tracking-tight ${getStatusColor()}`}>
                        {typeof value === 'number' ? value.toLocaleString() : value}
                    </h3>
                )}
                <p className="text-xs sm:text-sm text-zinc-500 font-medium">{title}</p>
                {subtitle && <p className="text-[10px] sm:text-xs text-zinc-400 dark:text-zinc-600">{subtitle}</p>}
            </div>
        </div>
    )
}
        ...
        <div className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 min-h-screen bg-zinc-50 dark:bg-black text-zinc-900 dark:text-zinc-100 font-sans">
                ...
                <>
                    ...
                    <div className="hidden sm:grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                        ...
                        <KPICard
                            ...
                            icon={<MemoryStick className="w-4 h-4 sm:w-5 sm:h-5" />}
                            loading={loading && !health}
                    ...
                    <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-xl overflow-hidden shadow-sm dark:shadow-none">
                        <div className="px-4 sm:px-6 py-4 border-b border-zinc-200 dark:border-zinc-800/50 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                            <div>
                                <h3 className="text-base sm:text-lg font-medium text-zinc-800 dark:text-zinc-200">System Status</h3>
                                <p className="text-xs text-zinc-500 mt-0.5">Detailed health information</p>
                            </div>
                            <span className={`text-xs font-medium px-2.5 py-1 rounded-full border w-fit ${health?.status === 'healthy'
                        ...
                        {loading && !health ? (
                            ...
                            <>
                                ...
                                <div className="md:hidden divide-y divide-zinc-200 dark:divide-zinc-800/50">
                                    {[
                                        { name: 'Database', icon: Database, color: 'text-blue-400', status: health?.database?.status, detail: health?.database?.message || 'PostgreSQL' },
                                        { name: 'Memory', icon: MemoryStick, color: 'text-purple-400', status: health?.memory?.status, detail: `${((health?.memory?.used || 0) / 1024 / 1024).toFixed(0)} MB used` },
                                        { name: 'WhatsApp', icon: Activity, color: 'text-emerald-400', status: health?.whatsapp?.status, detail: `${health?.whatsapp?.connected_bots || 0}/${health?.whatsapp?.total_bots || 0} bots` },
                                        { name: 'Redis', icon: Server, color: 'text-red-400', status: health?.redis?.status, detail: health?.redis?.message || 'Cache' },
                                    ].map((item, i) => (
                                        <div key={i} className="p-4 flex items-center justify-between gap-3">
                                            ...
                                            <span className={`px-2 py-0.5 rounded text-[10px] font-medium ${getHealthStatus(item.status || 'unknown') === 'healthy'
            ...
            {activeTab === 'backup' && (
                <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-xl overflow-hidden shadow-sm dark:shadow-none">
                    <div className="px-4 sm:px-6 py-4 border-b border-zinc-200 dark:border-zinc-800/50 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                        <div>
                            <h3 className="text-base sm:text-lg font-medium text-zinc-800 dark:text-zinc-200">Backup & Restore</h3>
                            <p className="text-xs text-zinc-500 mt-0.5">Manage database backups</p>
                        </div>
                        <button
                            onClick={handleCreateBackup}
                            disabled={loading}
                            className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white font-medium rounded-lg transition-all disabled:opacity-50 text-sm w-fit"
                        >
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-16">
                            ...
                            <h3 className="text-base sm:text-lg font-medium text-zinc-800 dark:text-zinc-300 mb-1">No backups found</h3>
                            <p className="text-zinc-500 text-xs sm:text-sm">Create your first backup to get started</p>
                        </div>
                    ) : (
                        <>
                            {/* Desktop Table View */}
                            ...
                            <div className="md:hidden divide-y divide-zinc-200 dark:divide-zinc-800/50">
                                {backups.map((backup) => (
                                    <div key={backup.id} className="p-4 flex items-center justify-between gap-3">
                                        <div className="min-w-0">
                                            <p className="text-sm font-medium text-zinc-900 dark:text-white truncate">{backup.filename}</p>
                                            <p className="text-[10px] text-zinc-500">
                                                {(backup.file_size / 1024 / 1024).toFixed(2)} MB • {backup.backup_type}
                                            </p>
                                            <p className="text-[10px] text-zinc-500">
            ...
            {activeTab === 'maintenance' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
                    <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-xl p-4 sm:p-6 group hover:border-zinc-300 dark:hover:border-zinc-800 transition-colors shadow-sm dark:shadow-none">
                        <div className="p-2 bg-zinc-100 dark:bg-zinc-900 rounded-lg text-zinc-700 dark:text-zinc-100 w-fit mb-3 sm:mb-4 group-hover:bg-zinc-200 dark:group-hover:bg-zinc-800 transition-colors">
                            <Database className="w-4 h-4 sm:w-5 sm:h-5 text-blue-400" />
                        </div>
                        <h3 className="font-semibold text-sm sm:text-base text-zinc-900 dark:text-white mb-1 sm:mb-2">Optimize Database</h3>
                        <p className="text-xs sm:text-sm text-zinc-500 mb-3 sm:mb-4">Run VACUUM to optimize database performance and reclaim storage</p>
                        <button
                            onClick={handleOptimizeDB}
                            disabled={loading}
                            className="w-full px-3 sm:px-4 py-2 sm:py-2.5 bg-zinc-100 dark:bg-zinc-900 hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-900 dark:text-zinc-100 rounded-lg border border-zinc-200 dark:border-zinc-800 transition-colors disabled:opacity-50 font-medium text-xs sm:text-sm"
                        >
                    ...
                    <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-xl p-4 sm:p-6 group hover:border-zinc-300 dark:hover:border-zinc-800 transition-colors shadow-sm dark:shadow-none">
                        ...
                        <h3 className="font-semibold text-sm sm:text-base text-zinc-900 dark:text-white mb-1 sm:mb-2">Cleanup Logs</h3>
                        <p className="text-xs sm:text-sm text-zinc-500 mb-3 sm:mb-4">Delete old audit logs and system logs to free up space</p>
                        <button
                    ...
                    <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-xl p-4 sm:p-6 group hover:border-zinc-300 dark:hover:border-zinc-800 transition-colors shadow-sm dark:shadow-none sm:col-span-2 md:col-span-1">
                        <div className="p-2 bg-zinc-100 dark:bg-zinc-900 rounded-lg text-zinc-700 dark:text-zinc-100 w-fit mb-3 sm:mb-4 group-hover:bg-zinc-200 dark:group-hover:bg-zinc-800 transition-colors">
                            <Zap className="w-4 h-4 sm:w-5 sm:h-5 text-amber-400" />
                        </div>
                        <h3 className="font-semibold text-sm sm:text-base text-zinc-900 dark:text-white mb-1 sm:mb-2">Clear Cache</h3>
                        <p className="text-xs sm:text-sm text-zinc-500 mb-3 sm:mb-4">Clear all cached data and refresh system state</p>
                        <button

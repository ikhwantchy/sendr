...
class TemplateEngineService {
    ...
    processTemplate(template, variables) {
        ...
        processed = processed.replace(/{TODAY}/gi, (0, date_fns_1.format)(now, 'dd/MM/yyyy'));
        processed = processed.replace(/{TODAY_DATE}/gi, (0, date_fns_1.format)(now, 'dd/MM/yyyy'));
     ...
     * Footer text
     */
    renderGeneralTemplate(template, items, globalVars = {}) {
        let processed = template;
     ...
     */
    generateAcademicDigestVariables(scheduleData, tasksData, timezone = 'Asia/Jakarta') {
        const now = new Date();
        const zonedDate = (0, date_fns_tz_1.toZonedTime)(now, timezone);
        // Get today's info
        const todayDate = (0, date_fns_1.format)(zonedDate, 'dd/MM/yyyy');
        const dayNames = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
     ...
     */
    filterTodaySchedule(scheduleData, todayName) {
    ...
    formatSchedule(scheduleData) {
        ...
        return scheduleData.map((item, index) => {
            ...
            if (waktu)
                text += `\n${waktu}`;
            if (ruang)
                text += ` (${ruang})`; // Optional: Add room if available
            if (dosen)
                text += `\n${dosen}`;
            return text;
        }).join('\n\n');
     ...
     */
    filterUrgentTasks(tasksData, currentDate) {
    ...
    formatTasks(tasksData) {
        ...
        return tasksData.map((item, index) => {
            ...
            let details = [];
            if (kelompok)
                details.push(kelompok);
            if (materi)
                details.push(materi);
            if (deskripsi)
                details.push(deskripsi);
            if (details.length > 0) {
                text += `\n${details.join(' - ')}`;
            }
            return text;
     ...
     */
    validateTemplate(template, availableVariables) {
...
exports.default = new TemplateEngineService();
//# sourceMappingURL=templateEngineService.js.map
...
import {
    Users as UsersIcon, User, UserPlus, Bot, Plus, X,
    RefreshCw, Search, ChevronDown, Shield, Eye, Pencil, Trash2, Loader2, Key
} from 'lucide-react'
    ...
    return (
        <AdminGuard>
            <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 lg:space-y-8 min-h-screen bg-black text-zinc-100 font-sans">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                        ...
                        <div className="relative">
                            ...
                            <div
                                ...
                                className="hidden absolute top-full right-0 mt-2 w-36 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl z-50 overflow-hidden py-1"
                            >
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2 text-xs hover:bg-zinc-800 transition-colors ${roleFilter === option.value ? 'text-blue-400 bg-blue-400/5' : 'text-zinc-100'}`}
                                    >
                                        {option.label}
                                    </button>
                                ))}
                        ...
                        <button
                            onClick={() => refetch()}
                            disabled={isFetching}
                            className={`p-2 bg-zinc-900 border rounded-lg transition-all ${isFetching
                ...
                {/* Search Bar - Standalone without container */}
                <div className="relative">
                    <input
                        type="text"
                        value={searchQuery}
                        ...
                        className="w-full px-4 py-3 pl-11 bg-zinc-950 border border-zinc-900 rounded-xl text-zinc-100 text-sm focus:outline-none focus:border-zinc-700 placeholder:text-zinc-600"
                    />
                ...
                <div className="bg-zinc-950 border border-zinc-900 rounded-xl overflow-hidden">
                    {isLoading ? (
                        ...
                        <div className="text-center py-16">
                            ...
                            <h3 className="text-lg font-semibold text-zinc-100 mb-2">No Users Found</h3>
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b border-zinc-800/50 bg-zinc-900/30">
                                            <th className="text-left py-3 px-4 pl-6 text-xs font-medium text-zinc-500 uppercase tracking-wider">Email</th>
                                            ...
                                            <th className="text-center py-3 px-4 text-xs font-medium text-zinc-500 uppercase tracking-wider">Bots</th>
                                            <th className="text-right py-3 px-4 pr-6 text-xs font-medium text-zinc-500 uppercase tracking-wider">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {paginatedUsers.map((user: any) => (
                                            <tr key={user.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/50 transition-colors">
                                                <td className="py-4 px-4 pl-6 text-zinc-100 text-sm">{user.email}</td>
                                                <td className="py-4 px-4">
                                                    <span className="text-zinc-100 font-medium">{user.name || 'N/A'}</span>
                                                </td>
                                                <td className="py-4 px-4">
                                                ...
                                                <td className="py-4 px-4 text-center">
                                                    <span className="text-zinc-100 font-mono text-sm">{user.bots_count || 0}</span>
                                                </td>
                                                        ...
                                                        <button
                                                            onClick={() => {
                                                                if (user.password_plain) {
                                                                    toast.info(`Password for ${user.name || user.email}`, {
                                                                        description: user.password_plain,
                                                                        duration: 5000,
                                                                    })
                                                                } else {
                                                                    toast.error('Password not available', {
                                                                        description: 'This is a legacy user. Please reset their password to enable viewing.'
                                                                    })
                                                                }
                                                            }}
                                        ...
                                        {Array.from({ length: Math.max(0, itemsPerPage - paginatedUsers.length) }).map((_, index) => (
                                            <tr key={`empty-${index}`} className="border-b border-zinc-800/50 h-[60px]">
                                                <td className="py-4 px-4 pl-6">{'\u00A0'}</td>
                                                <td className="py-4 px-4">{'\u00A0'}</td>
                                                <td className="py-4 px-4">{'\u00A0'}</td>
                                                <td className="py-4 px-4 text-center">{'\u00A0'}</td>
                                                <td className="py-4 px-4 pr-6">{'\u00A0'}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                ...
                {showAddBotModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                        <div className="w-full max-w-md bg-zinc-950 border border-zinc-900 rounded-xl overflow-hidden shadow-2xl">
                            <div className="px-6 py-4 border-b border-zinc-800/50 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-medium text-white">Create Bot for Client</h3>
                                        <p className="text-xs text-zinc-500">Assign to {selectedUser?.name}</p>
                                    </div>
                                </div>
                                <button onClick={() => setShowAddBotModal(false)} className="p-2 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-lg transition-all">
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                            <div className="p-6 space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-zinc-900 border border-zinc-800 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-zinc-700"
                                    />
                                </div>
                            </div>
                            <div className="px-6 py-4 border-t border-zinc-800/50 flex gap-3">
                                ...
                                <button
                                    ...
                                    className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium text-sm transition-colors disabled:opacity-50"
                                >

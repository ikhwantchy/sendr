...
const aiSheetUpdaterService_1 = require("../../services/aiSheetUpdaterService");
...
class AIEngine {
    ...
    initialize() {
        ...
        eventBus_1.eventBus.subscribe(types_1.EventType.KEYWORD_NO_MATCH, this.handleNoMatch.bind(this));
    ...
    async handleMessageReceived(event) {
        ...
        logger_1.logger.info('[AIEngine] handleMessageReceived called', { bot_id, contact_id, from: payload.from, content: payload.content?.substring(0, 30) });
        try {
            // Get bot AI config first to check mode
            const botResult = await (0, connection_1.query)('SELECT ai_config, name FROM bots WHERE id = ?', [bot_id]);
            ...
            const targetConfigResult = await (0, connection_1.query)('SELECT llm_config FROM llm_allowed_targets WHERE bot_id = ? AND target_jid = ? AND is_enabled = 1 LIMIT 1', [bot_id, targetJid]);
            ...
            logger_1.logger.info('[AIEngine] Sheet confirmation check', {
                targetJid,
            ...
            if (sheetResult?.success && sheetResult.mode === 'create' && sheetResult.extractedData && shouldSendConfirmation) {
                ...
                if (dataEntries) {
                    ...
                    logger_1.logger.info('[AIEngine] Sending sheet create confirmation', { bot_id, confirmation: confirmationMessage });
            ...
            if (aiConfig.mode === 'data_collection' || aiConfig.mode === 'hybrid') {
                if (aiConfig.dataSchema?.enabled) {
                    const result = await llmService_1.llmService.extractData(bot_id, payload.content, contact_id);
                    // If data is complete, we might want to trigger a confirmation or save to sheet
                    if (result.isComplete) {
                        logger_1.logger.info('AI data extraction complete', { bot_id, contact_id, data: result.extracted });
            ...
            logger_1.logger.error('AI handleMessageReceived error', { error, bot_id });
        }
    }
    /**
    ...
    async handleNoMatch(event) {
        ...
        logger_1.logger.info('AI handleNoMatch triggered', { bot_id, contact_id, group_id: context.group_id, content: userMessage });
        try {
            // Get bot AI config
            const botResult = await (0, connection_1.query)('SELECT ai_config, name, phone_number, lid FROM bots WHERE id = ?', [bot_id]);
            if (!botResult.rows.length)
                return;
            ...
            if (!isAllowed) {
                logger_1.logger.debug('AI fallback skipped: sender not in whitelist', {
                    bot_id,
                    senderJid,
            ...
            if (isMentioned || isPrivate) {
                ...
                try {
                    ...
                    logger_1.logger.warn('AI failed, sending fallback response', { error: aiError.message, bot_id });
                    let fallbackMessage = '';
     ...
     */
    isBotMentioned(payload, botName, phoneNumber, lid) {
        const message = payload.content || '';
        const mentionedJids = payload.mentioned_jids || [];
        const normalized = message.toLowerCase();
        logger_1.logger.info('AI Mention check detail', {
            botName,
            phoneNumber,
            lid,
            mentionedJids,
        ...
        if (phoneNumber || lid) {
            ...
            if (mentionedJids.some(jid => {
                ...
                logger_1.logger.info('AI Mention detected: Found in mentioned_jids');
                return true;
            }
        }
        ...
        if (payload.quoted_message?.participant && (phoneNumber || lid)) {
            ...
            if ((cleanPhone && quotedParticipant === cleanPhone) || (cleanLid && quotedParticipant === cleanLid)) {
                logger_1.logger.info('AI Mention detected: Quoted message reply');
                return true;
            }
        }
        ...
        const checkPatterns = [];
        if (lid)
            checkPatterns.push(`@${lid.replace(/\D/g, '')}`);
        if (phoneNumber)
            checkPatterns.push(`@${phoneNumber.replace(/\D/g, '')}`);
        if (botName) {
        ...
        for (const pattern of checkPatterns) {
            if (normalized.includes(pattern)) {
                logger_1.logger.info('AI Mention detected: Text pattern match', { pattern });
                return true;
            }
        }
        // 4. Generic @bot/@ai mention in text
        if (normalized.includes('@bot') || normalized.includes('@ai')) {
            logger_1.logger.info('AI Mention detected: Generic @bot/@ai');
            return true;
        }
        return false;
    }
    ...
    async isTargetAllowed(botId, targetJid) {
        try {
            ...
            logger_1.logger.debug('LLM whitelist check', {
                botId,
                targetJid,
                isAllowed,
            ...
            logger_1.logger.error('Error checking LLM whitelist', { error, botId, targetJid });
     ...
     */
    async processSheetUpdate(botId, senderJid, message, payload, resolvedPhone) {
        logger_1.logger.info('[SheetUpdater] 🔄 Starting processSheetUpdate', { botId, senderJid, message: message?.substring(0, 30), resolvedPhone, senderName: payload?.sender_name });
        try {
            ...
            if (!phone) {
                ...
                if (senderJid.includes('@lid')) {
                    ...
                    logger_1.logger.info('[SheetUpdater] 🔍 Looking up LID mapping', { botId, lid });
                    const mappedPhone = await lidPhoneMappingService_1.lidPhoneMappingService.getPhoneByLid(botId, lid);
                    logger_1.logger.info('[SheetUpdater] 📱 LID lookup result', { lid, mappedPhone });
                    if (mappedPhone) {
                        phone = mappedPhone;
                        logger_1.logger.info('[SheetUpdater] Resolved LID to phone from mapping', { lid, phone });
                    }
                    else {
                        // LID not in mapping - will try to match by name in processMessage
                        logger_1.logger.warn('[SheetUpdater] ⚠️ Unknown LID - will try name match', {
                            botId,
                            lid,
                            senderName,
            ...
            logger_1.logger.info('[SheetUpdater] Processing message', { botId, phone, resolvedPhone, senderJid, senderName, message: message?.substring(0, 50) });
            if (!phone || !message) {
                logger_1.logger.warn('[SheetUpdater] Missing phone or message, skipping', { phone, message });
                return null;
            }
            logger_1.logger.info('[SheetUpdater] 🚀 Calling aiSheetUpdaterService.processMessage', { botId, phone, senderName, message: message?.substring(0, 30) });
            const result = await aiSheetUpdaterService_1.aiSheetUpdaterService.processMessage(botId, phone, message, senderJid, senderName // Pass sender name for name-based matching
            );
            logger_1.logger.info('[SheetUpdater] 📊 processMessage result', { result });
            if (result.success) {
                logger_1.logger.info('📊 AI Sheet Update successful', {
                    botId,
                    phone,
                ...
                logger_1.logger.debug('[SheetUpdater] No update made', { reason: result.message, phone });
            }
            return null;
        }
        catch (error) {
            logger_1.logger.error('AI Sheet Update error', { error: error.message, botId, senderJid });
            return null;
        }
    }
}

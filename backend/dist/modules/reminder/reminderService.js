...
class ReminderService {
    ...
    async createReminder(data) {
        const { tenant_id, bot_id, name, message, group_id, schedule_type, schedule_config, } = data;
        try {
            // Verify group exists and is active
            const groupResult = await (0, connection_1.query)('SELECT * FROM wa_groups WHERE id = ? AND bot_id = ? AND is_active = 1', [group_id, bot_id]);
            ...
            // Insert reminder
            await (0, connection_1.query)(`INSERT INTO reminders (
            ...
            logger_1.logger.error('Failed to create reminder', { error: error.message });
            throw error;
        }
    }
    /**
     ...
     */
    async scheduleReminder(reminderId, scheduleType, config, nextRunAt) {
        try {
            if (scheduleType === 'once') {
                // One-time reminder - delay job
                const delay = nextRunAt.getTime() - Date.now();
                await messageQueue_1.messageQueue.add('reminder-message', {
                    type: 'reminder',
                    reminder_id: reminderId,
                }, {
            ...
            logger_1.logger.info('Reminder scheduled with Bull', {
                reminder_id: reminderId,
                schedule_type: scheduleType,
            });
        }
        catch (error) {
            logger_1.logger.error('Failed to schedule reminder', { error: error.message });
            throw error;
        }
    }
    /**
    ...
    calculateNextRun(scheduleType, config) {
        try {
            ...
            logger_1.logger.error('Failed to calculate next run', { error: error.message });
            return null;
        }
    }
    /**
     * List reminders
     */
    async listReminders(tenantId, botId) {
        try {
            let sql = `
            ...
            logger_1.logger.error('Failed to list reminders', { error: error.message });
            throw error;
        }
    }
    /**
    ...
    async getReminder(reminderId) {
        try {
            ...
            logger_1.logger.error('Failed to get reminder', { error: error.message });
            throw error;
        }
    }
    /**
    ...
    async toggleReminder(reminderId, isActive) {
        try {
            await (0, connection_1.query)("UPDATE reminders SET is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [isActive ? 1 : 0, reminderId]);
            logger_1.logger.info('Reminder toggled', {
                reminder_id: reminderId,
                is_active: isActive,
            });
        }
        catch (error) {
            logger_1.logger.error('Failed to toggle reminder', { error: error.message });
            throw error;
        }
    }
    /**
    ...
    async deleteReminder(reminderId) {
        try {
            ...
            await (0, connection_1.query)('DELETE FROM reminders WHERE id = ?', [reminderId]);
            logger_1.logger.info('Reminder deleted', { reminder_id: reminderId });
        }
        catch (error) {
            logger_1.logger.error('Failed to delete reminder', { error: error.message });
            throw error;
        }
    }
    /**
     ...
     */
    async updateLastRun(reminderId) {
        try {
            await (0, connection_1.query)(`UPDATE reminders 
                SET last_run_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP 
                WHERE id = ?`, [reminderId]);
        }
        catch (error) {
            logger_1.logger.error('Failed to update last run', { error: error.message });
        }
    }
}
